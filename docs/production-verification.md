...
- [ ] Android `1.2.3` (`versionCode 8`) получен как
      ...
      его SHA-256 равен
      `f57cf09e1dd71c219ff7206ad0507310cf77a545fd976350a166df0b69c69e70`, а
...
- [ ] `/srv/amigo/data/android/amigo-sync.apk` — root:root regular file `0600`
      с точным hash `1.2.3`; `web` видит `/android` только read-only.
...
- [ ] Pressure/heart/SpO2/VO2 evidence поддерживает только совет повторить
      ...
      по калориям.
- [ ] Signed APK `1.2.3` установлен через `adb install -r`; прежние pairing
...
- [ ] Release `v5.0.4` указывает на deployed feature commit; asset
      `Amigo-1.2.3.apk` скачивается, повторно даёт ожидаемые APK SHA-256 и
