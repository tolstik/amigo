...
- [ ] Явно выбран один режим: владелец разрешил помеченный Telegram smoke и
      сообщение пришло в правильный чат либо использован
      `--skip-telegram-test` и тестовое сообщение не отправлялось. Старые
      измерения при полном импорте не отправлены.
- [ ] При повторном релизе прежний v2 worker остановлен до migration/one-shot
      sync; нет параллельной ротации OAuth двумя worker-процессами.
...
- [ ] CSV export, фильтры периода, светлая/тёмная тема и табличная альтернатива
      графикам работают.
- [ ] Desktop и mobile: на «Прогрессе» видны оба недельных графика — «Вес по
      неделям» (парные столбики «Факт/План» и линия минимума) и «Изменение по
      неделям» (парные столбики «Факт/План»); отрицательная дельта означает
      снижение, положительная — набор.
- [ ] Недельная таблица совпадает с tooltip и
      `/amigo/api/v1/series/weight?range=program`: границы ISO-недель считаются
      Mon–Sun в `Europe/Moscow`, первая неделя обрезана датой 15.08.2026,
      текущая завершается локальной датой `as_of` и остаётся помеченной
      как неполная до начала следующей ISO-недели.
      Пустые недели сохранены с пустым фактом; выборочно пересчитанные дневные
      медианы, план, минимум, дельта и счётчики совпадают с API. Таблица и
      горизонтальная прокрутка читаемы на desktop и mobile.
