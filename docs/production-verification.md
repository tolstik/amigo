...
- [ ] Desktop и mobile: «Обзор», «Прогресс», «Вся история», «Давление»,
      «Состав тела», «Активность», «Бассейн», «Восстановление», «Качество данных»,
      «Задачи», «Анализы», «Исследования», «Пакет для
...
- [ ] Heart metrics, SpO2 и VO2 max на детерминированных экранах показаны
      описательно, без severity-цветов и app-side диагнозов. На экране давления
      отдельный дневной график показывает точные домашние ориентиры: нейтральный
      ниже `90/60`, зелёный `90–134/60–84`, жёлтый `135–179` или `85–119`,
      красный `>=180` или `>=120`; категория дня выбирает наиболее требующую
      внимания сессию в порядке «домашний ориентир → ниже ориентира →
      повышенное → критически высокое», названа визуальным ориентиром, а красное
      пояснение предлагает повторить измерение и при симптомах обратиться за
      экстренной помощью. Основной график давления отдельно подписывает
      пунктирные границы `135/85` и `180/120`.
...
- [ ] На «Активности» дневные шаги/активные минуты и недельные
      ...
      временную шкалу с видимым управлением детализацией/масштабом, корректно
      агрегирует min/weighted avg/max и разрывает линию на интервалах без данных.
...
- [ ] Корреляции показываются только при достаточном перекрытии; адаптивные
      карточки не обрезаются на desktop/mobile, расшифровывают диапазон, знак и
      модуль коэффициента Пирсона `r` и явно предупреждают, что корреляция не
      доказывает причину.
...
- [ ] Сравнение анализов отсутствует в меню и Android allowlist; GET/POST
      `/amigo/api/v1/labs/compare` возвращают 404. Архив, результаты и история
      отдельных лабораторных показателей доступны.
- [ ] «Бассейн» показывает только завершённое покрытие активного Xiaomi:
      сводку с количеством известных значений, графики и до 50 тренировок на
      странице. Неопределённое плавание и открытая вода исключены. Общий период
      сохраняется; прочерки не превращаются в нулевую дистанцию.
...
- [ ] Routine 3-day и weekly 30-day reconciliation используют отдельный
      resumable recent lane с одним persisted target/width для всех десяти
      метрик и продолжаются во время historical backfill. Незавершённый legacy
      historical cursor сохранил exact range/page token/page index/hash state;
      recent final coverage опубликовалось, после чего historical page
      продолжился с прежнего места.
...
- [ ] Незавершённый backfill добавляет continuation через `APPEND_OR_REPLACE` и
      ...
      `chrome-error://` страницу за внешний небезопасный адрес. Continuation
      завершает уже persisted recent round, но не открывает новый routine round
      и не задерживает текущие шаги до достижения historical floor.
