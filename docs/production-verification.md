...
- [ ] Android `1.3.3` (`versionCode 13`) получен как
      ...
      его SHA-256 равен
      `6f4156d6cf24df27b95b6cc53b26f83bd965c266da144e04f6feb3ccb884f156`, а
...
- [ ] `/srv/amigo/data/android/amigo-sync.apk` — root:root regular file `0600`
      с точным hash `1.3.3`; `web` видит `/android` только read-only.
...
- [ ] Pressure/heart/SpO2/VO2 evidence поддерживает только совет повторить
      ...
      по калориям.
- [ ] Signed APK `1.3.3` установлен через `adb install -r`; прежние pairing
...
- [ ] Release `v5.1.4` указывает на deployed feature commit; asset
      `Amigo-1.3.3.apk` скачивается, повторно даёт ожидаемые APK SHA-256 и
