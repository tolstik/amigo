...
## Проверка недельного и месячного прогресса

Графики `/amigo/progress` показывают факт по конечным дневным медианам,
план на дату и фиксированный план на весь календарный период. Оба плана
следуют исходной линии −4 кг между 15-ми числами: календарный сентябрь 2026
имеет полный план −3,931 кг. Первый период использует 127,03 кг; отсутствие
замера на предыдущий календарный день переключает факт на первый замер
внутри периода. Даты и веса сравнения видны в подсказках и таблице.
Цвет сравнивает факт с планом за ровно те же даты замеров, без экстраполяции.
...
на график истории. Обзор показывает границы московских месяцев и 3D-фигуру
«Старт / Сейчас / Цель»: 127,03 кг, последний замер Withings с датой и 76,5 кг.
При отсутствии замера средняя позиция остаётся без фигуры. Переключение позы,
ракурса, одежды и аксессуаров не сохраняет замеры. В скрытой вкладке, вне экрана
и при снижении движения анимация приостанавливается.

Персональная голова, атлас лица и текстура уха хранятся в необязательном
версионированном пакете `user_body_face` в PostgreSQL.
`GET /api/v1/profile/body-model-assets` требует сессию и отдаёт no-store JSON
или 404. Пакет содержит только проверенную геометрию и PNG-пиксели; исходный HTML
не исполняется. Атлас лица сохраняет разрешение 2048×1024, нужное для UV.
При отсутствии пакета клиент показывает нейтральную голову. Прежняя PNG-текстура
и `GET /api/v1/profile/body-face` остаются для совместимости с предыдущим
релизом.

Для выпуска персональной модели Claude в production оператор до запуска
релиза переносит приватный HTML в `/home/tolstik/amigo-body-model.html` на
production host: обычный файл, владелец `tolstik`, права строго `0600`, размер
не более 2 МиБ. Если пакет ещё не сохранён в БД и staging отсутствует,
проверка допускает 404, а клиент покажет нейтральную голову. Файл должен
находиться вне Git, образов и публичной статики. Штатный `deploy.sh` после
миграций читает его с `O_NOFOLLOW`, проверяет владельца, права и размер, затем
передаёт байты только в stdin `python -m app.body_model_assets` контейнера `web`.
Импорт валидирует ресурсы и идемпотентно сохраняет нормализованный пакет в БД;
логи не содержат HTML, изображений или геометрии. Проверка production читает
только статус/контракт без вывода тела ответа. После успешного checkpoint deploy
удаляет staging-файл только если его `dev:inode` совпадает с записанным до
импорта; заменённый файл остаётся нетронутым. При ошибке импорта staging
сохраняется для повторной попытки.

Прежний явный импорт PNG по `/home/tolstik/amigo-body-face.png` также остаётся:
до 512 КиБ/1024×1024, владелец `tolstik`, права `0600`. Его bounded reader
передаёт изображение через stdin `app.body_face`; после успешного deploy staging
удаляется с той же проверкой `dev:inode`. Персональные ресурсы исключены из AI,
отчётов, CSV, Telegram и source checkpoint. Исходные файлы остаются у владельца
вне репозитория.
...
режим `0600` и pinned hash. Smoke передаёт только вымышленные проверочные данные:
31 факт, 10 рядов по 28 дней и 24 лабораторные строки. Он проверяет схему,
точные evidence keys, лабораторную оценку и обязательные ограниченные действия
при медицинских данных и лабораторных отклонениях. Затем по одному разу
проверяются extraction и analyte guides; assistant допускает ровно один повтор
невалидного ответа.
...
Регулярный анализ сохраняет прежние детерминированные факты, включая сглаженный
вес и плановое отклонение. Только его дневные ряды ограничены последними 28
московскими календарными днями (`...28d`, без заполнения пропусков). Из прежнего
ограниченного набора 240 лабораторных кандидатов берётся последнее значение
каждого сочетания показателя, биоматериала, метода и единицы в пределах 90 дней
от самой свежей лабораторной даты; отклонения от указанного референса имеют
приоритет, общий предел — 24 результата. Непроверенные результаты сохраняют свой
статус. Полная отметка свежести источников не уменьшается, а архив и контекст
постоянного помощника продолжают использовать прежнюю историю.

Для Codex повторяющиеся названия полей передаются таблицами `columns`/`rows`.
...
Codex повторял его и отображал как разрыв потока, включая переход с WebSocket
на HTTPS. Снижение verbosity/reasoning и отключение сжатия это не устранили;
...
6. Нажать «Синхронизировать сейчас» и дождаться успешного status. В версии
   ...
   с прежними ID, страницами и хешами. Запуск обрабатывает до 40 страниц,
   прекращая начало новых страниц через 90 секунд; 8 из 10 попыток отдаются
   свежим данным, по одной — месячной сверке и истории, свободная доля
   используется доступной работой. После задержанного пробуждения снова
   проверяется свежесть последних дней. Экран отдельно показывает дату
   завершённой сверки шагов и незавершённую догрузку.
...
## Full and elapsed period plan release

- Deployed `be2720e6f0f5a5da25840f9d1b6d578522ce78bf` on 2026-09-11. Progress weekly/monthly change charts separate observed endpoint change, elapsed calendar plan, and fixed full-period plan. Weekly weight shows the latest daily median and both planned weights. Full calendar September is −3.931 kg on the unchanged matching-month-day plan; August uses the program baseline. Missing boundaries and observed dates remain explicit.
- CI passed: `https://github.com/tolstik/amigo/actions/runs/34579246739`. Local checks: 86 backend/release/API/report/snapshot tests, 62 frontend tests, and desktop/mobile progress browser checks. Recovery-transition guards passed.
- Production verification passed, including weekly/monthly endpoint and full/elapsed plan contracts, seven healthy services, post-start Withings success, authentication/CSRF, immutable evidence, privacy-bounded temporary PDF deletion, and SSE. Deployment used `--skip-telegram-test` with automatic previous-release recovery.
- Root-owned production checkpoint commit: `eb5246b682b4689c9c33f5ed5fc3dabcd51e8d99`; durable ref: `refs/amigo/checkpoints/be2720e6f0f5a5da25840f9d1b6d578522ce78bf`. Runtime identity comes from `/var/lib/amigo/current-release`.
- Frontend `index-BLUoHWi0.js` SHA-256: `e4fe5c916e9d2a01d173ed6f8ab1490fa08f65ad13449da6546c672f472dd933`.
- Frontend `index-CJwOAEPX.css` SHA-256: `fea8c5daf4142eb671a1cb04d613790882645f3ae6296a27fe4825dc9eeeeaa7`.
