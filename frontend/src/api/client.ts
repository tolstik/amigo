...
function normalizeWeightPeriods(values: unknown[]): PeriodWeightPoint[] {
  return values
    .map((value): PeriodWeightPoint | null => {
      ...
      return {
        startDate,
        endDate,
        periodEndDate: string(value, "period_end_date", "periodEndDate") ?? endDate,
        planStartDate: string(value, "plan_start_date", "planStartDate") ?? startDate,
        plannedStartKg: number(value, "planned_start_kg", "plannedStartKg"),
        plannedEndKg: number(value, "planned_end_kg", "plannedEndKg"),
        plannedToDateKg: number(value, "planned_to_date_kg", "plannedToDateKg"),
        plannedFullChangeKg: number(value, "planned_full_change_kg", "plannedFullChangeKg"),
        actualStartDate: string(value, "actual_start_date", "actualStartDate"),
        actualEndDate: string(value, "actual_end_date", "actualEndDate"),
        actualStartKg: number(value, "actual_start_kg", "actualStartKg"),
        actualEndKg: number(value, "actual_end_kg", "actualEndKg"),
        plannedObservedChangeKg: number(value, "planned_observed_change_kg", "plannedObservedChangeKg"),
    ...
    .filter((point): point is PeriodWeightPoint => point !== null)
...
export function normalizeWeightSeries(payload: unknown, range: Period): WeightSeriesResponse {
  ...
  const raw = list(body, "raw")
    ...
    .sort((left, right) => left.measuredAt.localeCompare(right.measuredAt));
  const weekly = normalizeWeightPeriods(list(body, "weekly", "weeks"));
  const monthly = normalizeWeightPeriods(list(body, "monthly", "months"));
  return { points, raw, projection, planProjection, weekly, monthly, meta: metaWithBounds(payload, range, points) };
}
