...
describe("daily weight candles", () => {
  it("spans the full change between single daily weighings, including the anchor outside the week", () => {
    const candles = dailyWeightCandles([
      { measuredAt: "2026-08-31T06:00:00Z", valueKg: 125.9 },
      { measuredAt: "2026-09-01T06:00:00Z", valueKg: 125.4 },
      { measuredAt: "2026-09-02T06:00:00Z", valueKg: 125.6 },
    ], asOf);
    expect(candles.map(point => point.date)).toEqual(["2026-09-01", "2026-09-02"]);
    expect(candles[0]).toMatchObject({ previousKg: 125.9, previousDate: "2026-08-31", lastKg: 125.4, sampleCount: 1 });
    ...
    expect(option.tooltip.formatter([{ axisValue: "2026-09-01" }])).toContain("−0,50 кг");
  });
  ...
  it("orders mixed timezone timestamps and keeps daily extremes across Moscow midnight", () => {
    ...
    const option = dailyWeightChartOption(candles, asOf) as any;
    expect(option.series[0].data[0]).toEqual([126, 125.4, 125.2, 126.1]);
  ...
  it("leaves missing days empty and labels comparison to the actual previous measured day", () => {
    ...
    const option = dailyWeightChartOption(candles, asOf) as any;
    expect(option.series[0].data.slice(0, 3)).toEqual([["-", "-", "-", "-"], [126, 125.5, 125.5, 126], [125.5, 125.5, 125.5, 125.5]]);
  ...
  it("shows exactly seven Moscow calendar days ending today, regardless of stale or future measurements", () => {
    const midnight = "2026-09-06T21:00:00Z";
    const candles = dailyWeightCandles([
      { measuredAt: "2026-08-30T06:00:00Z", valueKg: 126 },
      { measuredAt: "2026-09-08T06:00:00Z", valueKg: 125 },
    ], midnight);
    const option = dailyWeightChartOption(candles, midnight) as any;
    expect(option.xAxis.data).toEqual(["2026-09-01", "2026-09-02", "2026-09-03", "2026-09-04", "2026-09-05", "2026-09-06", "2026-09-07"]);
  ...
  it("does not invent a change when there is no previous weighing", () => {
    ...
    const option = dailyWeightChartOption(candles, asOf) as any;
    expect(option.series[0].data[0]).toEqual([125.5, 125.5, 125.5, 125.5]);
