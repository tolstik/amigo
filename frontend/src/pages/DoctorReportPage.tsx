...
export function DoctorReportPage() {
  ...
  return <>
    ...
    {report && <article className="doctor-preview" aria-labelledby="doctor-preview-title">
      ...
      {report.preview.meta.excludedLabsCount > 0 && <aside className="report-labs-warning panel" role="status"><strong>{report.preview.meta.excludedLabsCount} лабораторных строк не включено</strong><p>Они ещё не подтверждены. Откройте «Анализы», сверяйте значения с бланками и нажмите «Всё проверено», затем сформируйте пакет заново.</p></aside>}
      ...
      {report.preview.labs && <section className="report-list panel" aria-labelledby="report-labs-title"><div className="panel__head"><div><span className="eyebrow">Только подтверждённые</span><h2 id="report-labs-title">Лабораторные результаты</h2></div></div>{report.preview.labs.length ? <div className="data-table-scroll"><table className="data-table"><thead><tr><th>Дата</th><th>Показатель</th><th>Значение</th><th>Референс</th><th>Статус</th></tr></thead><tbody>{report.preview.labs.map((item, index) => <tr key={`${item.analyte}-${item.observedOn}-${index}`}><td>{formatDate(item.observedOn)}</td><td>{item.analyte}</td><td>{item.value}</td><td>{item.reference ?? "—"}</td><td>{labStatusLabels[item.status]}</td></tr>)}</tbody></table></div> : <p>За выбранный период подтверждённых результатов нет.</p>}</section>}
