...
import { activityDailyChartOption, circumferenceChartOption, doctorPressureChartOption, sleepChartOption, watchHeartRateDailyChartOption, weightActualChartOption } from "../charts/options";
...
const sectionOptions: Array<{ key: DoctorReportSection; label: string; note: string }> = [
  ...
  { key: "recovery", label: "Сон и пульс с часов", note: "Сон в часах; пульс — минимум, среднее и максимум" },
  { key: "labs", label: "Анализы", note: "Проверенные и исходные результаты" },
  { key: "studies", label: "Исследования", note: "Только подтверждённые" },
];

const defaultSections = sectionOptions.map((item) => item.key);
...
export function DoctorReportPage() {
  ...
  return <>
    ...
    <form className="report-builder panel" onSubmit={create} aria-busy={creating}>
      ...
      <fieldset><legend>Разделы</legend><div className="report-sections">{sectionOptions.map((item) => <label key={item.key} className="report-section"><input type="checkbox" checked={sections.includes(item.key)} onChange={() => toggle(item.key)} /><span><strong>{item.label}</strong><small>{item.note}</small></span></label>)}</div></fieldset>
    ...
    {report && <article className="doctor-preview" aria-labelledby="doctor-preview-title">
      ...
      {report.preview.recovery && (report.preview.recovery.points.some((point) => point.averageHeartRateBpm !== null || point.minimumHeartRateBpm !== null || point.maximumHeartRateBpm !== null) ? <ChartCard title="Пульс с часов" subtitle="Дневные минимум, среднее и максимум · уд/мин" option={watchHeartRateDailyChartOption(report.preview.recovery.points)} ariaLabel="График минимального, среднего и максимального пульса с часов в пакете для врача" height={330} /> : <section className="report-empty panel"><h2>Пульс с часов</h2><p>За выбранный период данных с часов нет.</p></section>)}
